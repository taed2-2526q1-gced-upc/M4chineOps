...
# Mock preventivo para cv2 antes de importar el módulo
...
def tmp_dirs(tmp_path):
    """
    Fixture: crea directorios temporales simulando los conjuntos real/fake.
    """
    ...
    # Creamos videos falsos (archivos vacíos .mp4)
    for i in range(5):
...
def test_sample_split_creates_expected_structure(tmp_dirs, monkeypatch):
    """
    Test: sample_split genera carpetas, CSVs y metadatos esperados.
    """
    src_real, src_fake, dst_dir, metadata_dir = tmp_dirs

    # Mock de cv2.VideoCapture para no abrir videos reales
    mock_cap = MagicMock()
    ...
    # Verificamos estructura de salida
...
def test_main_creates_metadata_dir_and_calls_sample_split(tmp_path):
    """
    Test: main() crea METADATA_DIR si no existe y llama sample_split.
    """
